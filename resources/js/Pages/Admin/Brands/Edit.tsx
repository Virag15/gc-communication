...
import { Check, ArrowLeft, Image as ImageIcon, Upload, Loader2 } from "lucide-react";
...
export default function BrandEdit({ brand }: BrandEditProps) {
    ...
    const preview = useMemo(() => (data.logo ? URL.createObjectURL(data.logo) : brand.logo), [data.logo, brand.logo]);
    useEffect(() => {
        return () => {
            if (data.logo && preview) URL.revokeObjectURL(preview);
        };
    }, [data.logo, preview]);
    ...
    function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
        ...
        post(`/admin/brands/${brand.id}`, {
            ...
            onError: () => toast.error('Failed to update brand. Please check the form.'),
        });
    }

    return (
        <AdminLayout breadcrumbs={[{ label: 'Brands', href: '/admin/brands' }, { label: 'Edit' }]}>
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            <div className="min-w-0 space-y-1.5">
                                ...
                                <Button type="button" variant="outline" size="sm" onClick={() => inputRef.current?.click()}>
                                    ...
                                    {data.logo || brand.logo ? 'Change image' : 'Upload image'}
                                </Button>
                                <p className="truncate text-xs text-muted-foreground">
                                    {data.logo ? data.logo.name : brand.logo ? 'Current image set' : 'PNG, JPG, WebP or SVG'}
                                </p>
                            </div>
                        </div>
