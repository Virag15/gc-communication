...
import { type Brand, type Catalogue } from '@/types';
...
export default function CatalogueEdit({ catalogue, brands }: CatalogueEditProps) {
    ...
    const { data, setData, post, processing, errors } = useForm<{
        ...
        brand_id: catalogue.brand_id != null ? String(catalogue.brand_id) : '',
        file: null,
    ...
    function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
        e.preventDefault();
        post(`/admin/catalogues/${catalogue.id}`, {
            forceFormData: true,
            onSuccess: () => toast.success('Catalogue updated successfully.'),
            onError: () => toast.error('Failed to update catalogue. Please check the form.'),
        });
    }

    return (
        <AdminLayout breadcrumbs={[{ label: 'Catalogues', href: '/admin/catalogues' }, { label: 'Edit' }]}>
            <Head title="Edit Catalogue" />
            <div className="mb-6">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="space-y-4">
                    <div>
                        ...
                        {catalogue.file_name && (
                            <p className="text-sm text-muted-foreground mt-1">Current file: {catalogue.file_name}</p>
                        )}
                    </div>
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            <div className="min-w-0 space-y-1.5">
                                ...
                                <Button type="button" variant="outline" size="sm" onClick={() => fileInputRef.current?.click()}>
                                    ...
                                    {data.file ? 'Change file' : 'Upload new PDF'}
                                </Button>
                                <p className="truncate text-xs text-muted-foreground">
                                    {data.file ? data.file.name : 'Leave empty to keep the current file'}
                                </p>
                            </div>
                        </div>
