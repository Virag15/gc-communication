...
function getColumnLabel(columnDef: ColumnDef<unknown, unknown>) {
    if (columnDef.id === 'actions') return null;
    if (typeof columnDef.header === 'string') return columnDef.header;
    const key = ('accessorKey' in columnDef ? (columnDef.accessorKey as string) : undefined) || columnDef.id || '';
    if (!key) return null;
                            ...
                            {secondaryCells.length > 0 && (
                                <div className="space-y-1.5 text-sm">
                                    {secondaryCells.map((cell) => {
                                        const label = getColumnLabel(cell.column.columnDef as ColumnDef<unknown, unknown>);
                                ...
                                <>
                                    <PaginationItem>
                                        <PaginationPrevious
                                            ...
                                            onClick={(e) => {
                                                e.preventDefault();
                                                if (serverPagination!.prev_page_url) router.visit(serverPagination!.prev_page_url, { preserveState: true });
                                            }}
                                                ...
                                                <PaginationLink
                                                    ...
                                                    onClick={(e) => {
                                                        ...
                                                        if (page !== serverPagination!.current_page) {
                                                            const link = serverPagination!.links.find(l => l.label === String(page));
                                    ...
                                    <PaginationItem>
                                        <PaginationNext
                                            ...
                                            onClick={(e) => {
                                                e.preventDefault();
                                                if (serverPagination!.next_page_url) router.visit(serverPagination!.next_page_url, { preserveState: true });
                                            }}
